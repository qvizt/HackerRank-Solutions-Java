...
/*
 * NOTE: The whole solution is taken from 
 * Rodney Shaghoulian.
 * Github: github.com/rshaghoulian
 * HackerRank: hackerrank.com/rshaghoulian
 * 
 * The description of the challenge lacks of quite a lot
 * details, thus leading to dead ends that cost way too
 * much time.
 * 
 */
...
public class Solution {
    ...
    private static void addChildren(TreeNode parent, Integer parentNum) {
        ...
        for (Integer treeNum : map.get(parentNum)) {
            ...
            Tree tree;
            if (childHasChild) {
                tree = new TreeNode(values[treeNum - 1], colors[treeNum - 1],
                        parent.getDepth() + 1);
            } else {
                tree = new TreeLeaf(values[treeNum - 1], colors[treeNum - 1],
                        parent.getDepth() + 1);
            }
